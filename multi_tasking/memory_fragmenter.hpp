...
public:
    ...
    {         
        ...
        uint32_t availableSize = heapInfo.reserved_size - heapInfo.current_size - heapInfo.overhead_size;
        tr_debug("Available heap size is %d (reserved %d)", availableSize, heapInfo.reserved_size);
        
        // divide the available size by 8 blocks that we allocate
        uint32_t blockSize = (availableSize - kMarginSpace) / kNbrOfBlocks;
        tr_debug("Allocating blocks of size %d", blockSize);
        char *pBlockArray[kNbrOfBlocks] = {NULL};
        for (int blockIndex = 0; blockIndex < kNbrOfBlocks; blockIndex++) {
            pBlockArray[blockIndex] = new char[blockSize];
            if (pBlockArray[blockIndex] == NULL) {
                tr_error("Cannot allocate block memory for index %d", blockIndex);
            }
            tr_debug("Allocated block index %d of size %d at address 0x%08x", blockIndex, blockSize, (unsigned int) pBlockArray[blockIndex]);
        ...
        for (int blockIndex = 0; blockIndex < kNbrOfBlocks; blockIndex += 2) {
        ...
        heapInfo = {0};
        mbed_stats_heap_get(&heapInfo);
        availableSize = heapInfo.reserved_size - heapInfo.current_size - heapInfo.overhead_size;
        tr_debug("Available heap size is %d (reserved %d)", availableSize, heapInfo.reserved_size);
        blockSize += 8;
        // this allocation will fail
        tr_debug("Allocating 1 block of size %d should succeed !", blockSize);
